...
impl<const N: usize> Compiler<N> {
    ...
    fn compile_call(
        ...
        self.locals[function_index] = Local::Reserved;
        self.clear_all_to_depth()?;
        let mut arg_indices = Vec::with_capacity(arguments.len());
        for arg in arguments {
            let a_index = self.compile_non_tail_expression(arg)?;
            self.locals[a_index] = Local::Reserved;
            arg_indices.push(a_index);
            self.clear_all_to_depth()?;
        }
        ...
        if tail_position {
            self.chunk
                .opcodes
                .push(OpCode::TailCall(function_index.try_into()?));
            for a in arg_indices {
                self.chunk.opcodes.push(OpCode::CallArgument(a.try_into()?))
            }
            Ok(None)
        } else {
            let result_index = self.find_free_register()?;
            self.locals[result_index] = Local::Reserved;
            self.chunk.opcodes.push(OpCode::Call(
                function_index.try_into()?,
                result_index.try_into()?,
            ));
            self.locals[function_index] = Local::Temporary;
            for a in arg_indices {
                self.chunk.opcodes.push(OpCode::CallArgument(a.try_into()?));
                self.locals[function_index] = Local::Temporary;
            }
